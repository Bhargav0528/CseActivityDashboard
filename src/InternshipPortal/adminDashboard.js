...
class Internship extends React.Component{
	...
	componentDidMount(){
		...
		window.location.href="#internshipBody";
	...
	render(){
		...
		{(this.state.type=="open"||this.state.type=="application")?<div><b>Internship Description: </b><pre className="internship1">{this.state.description}</pre></div>:<div>
			<b className="internship1">Internship Description:</b><button id="internshipDescBtnAdminDashboard" onClick={this.expandInternshipDesc}>Expand</button><div id="internshipDescAdminDashboard">{this.state.description}</div></div>}
		...
		<td><b>Duration:</b> {this.state.duration}</td></tr>
		<tr><td><b>Interns Required:</b> {this.state.minRequirement==0?"Not Specified":this.state.minRequirement}</td></tr>
		</tbody></table>
		...
		{(this.state.type=="completed" && this.state.feedback)?<div className="internshipFeedbackDisp">
		<b>Feedback:</b>
			{this.state.feedback.map((item2,index2)=>{
				return(
				<div className="feedbackInternship" key={index2}>
				<h4 style={{padding:"0px", margin:"0px"}}>{item2.name}</h4>
				{item2.content}
				<div style={{width:"95%",margin:"10px 0px 0px 10px"}}>
				<ProgressBar percent={item2.score}  filledBackground="#FFDF00">
				<Step transition="scale">
				  {({ accomplished }) => (
					<img style={{ filter: `opacity(${accomplished ? 100 : 20}%)` }}  width="20" src="ratingstaricon.png"/>
				  )}
				</Step>				
				<Step transition="scale">
				  {({ accomplished }) => (
		...
		<a href={"tel:"+this.state.companyPhone}><button className="btn">Call Company</button></a><a href={"mailto:"+this.state.companyMail}><button className="btn">Email Company</button></a>
		...
		{((this.state.type=="ongoing" || this.state.type=="completed") && this.state.members && this.state.applied)?<div><b>Members:</b>
